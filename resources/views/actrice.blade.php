...
<body class="font-sans min-h-screen bg-white">
    ...
    <div class="flex flex-col md:flex-row md:h-screen">
        ...
        <div class="w-full md:w-1/2 flex flex-col justify-between p-6 md:p-16 relative bg-white">
            ...
            <nav class="flex justify-between items-center text-[10px] md:text-xs font-bold tracking-widest uppercase text-gray-800 mb-12 md:mb-0">
                ...
                <div id="google_translate_element" class="text-gray-800"></div>
            </nav>
    ...
    <section class="bg-gray-200 py-16 px-4 md:py-32 md:px-24">
        <div class="max-w-[1400px] mx-auto">
            <h2 id="biography" class="text-3xl md:text-6xl font-light text-center text-gray-800 uppercase tracking-[0.3em] mb-12 md:mb-40">
                BIOGRAPHIE
            </h2>
    ...
    <section class="py-16 px-4 md:py-[100px] md:px-0 bg-white" id="filmographie">
        <div class="container mx-auto px-4 max-w-7xl">
            <h2 id="filmography" class="text-3xl md:text-6xl font-light text-center text-gray-800 uppercase tracking-[0.3em] mb-12 md:mb-20">
                FILMOGRAPHIE
            </h2>
                    ...
                    <div class="w-[75vw] max-w-[320px] flex-shrink-0 snap-start">
                        <div class="flex flex-col">
                            <div class="aspect-[3/4] overflow-hidden rounded-[26px] mb-5 shadow-sm">
                                <img src="{{ Str::startsWith($work->image, 'img/') ? asset($work->image) : Storage::url($work->image) }}" alt="{{ $work->title }}" class="w-full h-full object-cover">
                            </div>
                            <div class="px-1">
    ...
    <section class="py-16 px-4 md:py-[100px] md:px-0 bg-white" id="actualite">
        <div class="container mx-auto px-4 max-w-7xl">
            <h2 id="actualite" class="text-3xl md:text-6xl font-light text-center text-gray-800 uppercase tracking-[0.3em] mb-12 md:mb-20">
                ACTUALITÉ / PRESSE
            </h2>
    ...
    <section class="bg-agenda-gradient py-16 px-4 md:py-32 md:px-24 text-white" id="agenda">
        <div class="max-w-[1200px] mx-auto">
            <h2 id="agenda" class="text-3xl md:text-6xl font-light text-center uppercase tracking-[0.3em] mb-12 md:mb-20">
                AGENDA
            </h2>
            ...
            <div class="border border-white/20 rounded-[30px] overflow-hidden backdrop-blur-sm bg-white/5">
                ...
                <div class="flex flex-col md:flex-row items-center p-8 md:p-12 border-b border-white/10 last:border-0 gap-8 md:gap-16">
                    ...
                    <div class="w-full md:w-64 aspect-[3/2] flex-shrink-0 rounded-2xl overflow-hidden shadow-lg">
                        <img src="{{ Str::startsWith($event->image, 'img/') ? asset($event->image) : Storage::url($event->image) }}" alt="{{ $event->title }}" class="w-full h-full object-cover">
                    </div>

                    <div class="flex-grow">
                        <h3 class="text-xl md:text-2xl font-bold uppercase tracking-tight mb-4 leading-tight">
                            {{ $event->title }}
                        </h3>
                        <p class="text-sm md:text-base font-light leading-relaxed opacity-70">
                            {{ $event->description }}
                        </p>
                    </div>
                </div>
    ...
    <section class="hidden md:block py-16 px-4 md:py-32 md:px-0 bg-white" id="teasers">
        <div class="container mx-auto px-4 max-w-7xl">
            <h2 id="teasers" class="text-3xl md:text-6xl font-light text-center text-gray-800 uppercase tracking-[0.3em] mb-12 md:mb-20">
                TEASERS
            </h2>
            ...
            <div id="teasers-scroller" class="-mx-4 px-4 flex gap-8 overflow-x-auto scroll-smooth snap-x snap-mandatory hide-scrollbar mb-16 py-8">
                ...
                <div class="w-[360px] flex-shrink-0 snap-start bg-white rounded-[30px] overflow-hidden shadow-lg border border-gray-100 group" data-teaser-card>
                    <div class="relative aspect-square">
                        <img src="{{ Str::startsWith($teaser->poster_image, 'img/') ? asset($teaser->poster_image) : Storage::url($teaser->poster_image) }}" alt="{{ $teaser->title }}" class="w-full h-full object-cover">
                        <div class="absolute bottom-6 right-6">
                            @php($teaserModalPayload = ['title' => $teaser->title, 'videoUrl' => $teaser->video_url])
                            <a href="#" data-teaser-modal='@json($teaserModalPayload)' class="w-16 h-16 bg-[#00818a] rounded-full flex items-center justify-center text-white shadow-xl hover:scale-110 transition-transform duration-300">
    ...
    <section class="py-16 px-4 md:py-32 md:px-0 bg-white text-center" id="booking">
        <div class="container mx-auto px-4 max-w-4xl">
            <h2 id="booking" class="text-3xl md:text-6xl font-light text-gray-800 uppercase tracking-[0.3em] mb-10 md:mb-12">
                BOOKING
            </h2>
    ...
    <script>
        (function () {
            ...
            var progressIndicator = document.getElementById('teasers-progress-indicator');
            ...
            function updateTeasersControls() {
                if (!scroller || !prevBtn || !nextBtn || !progressIndicator) return;
                ...
                var widthPct = (scroller.clientWidth / scroller.scrollWidth) * 100;
                var leftPct = (left / scroller.scrollWidth) * 100;
                progressIndicator.style.width = widthPct + '%';
                progressIndicator.style.transform = 'translateX(' + leftPct + '%)';
            }
            ...
            if (scroller && prevBtn && nextBtn) {
                ...
                scroller.addEventListener('scroll', updateTeasersControls, { passive: true });
                window.addEventListener('resize', updateTeasersControls);
                updateTeasersControls();
            }
            ...
            function openTeaserModal(payload) {
                if (!teaserModal || !teaserTitle || !teaserIframe || !teaserVideo) return;
                var title = payload.title || '';
                var url = payload.videoUrl || '';
                if (!url || url === '#') return;
                ...
                if (yt || vm) {
                    ...
                    teaserVideo.classList.add('hidden');

                    teaserIframe.src = yt || vm;
                    teaserIframe.classList.remove('hidden');
                } else {
                    teaserIframe.src = '';
                    teaserIframe.classList.add('hidden');

                    teaserVideo.src = url;
                    teaserVideo.classList.remove('hidden');
                    teaserVideo.play().catch(function () {});
                }

                teaserModal.classList.remove('hidden');
                teaserModal.classList.add('flex');
                document.body.classList.add('overflow-hidden');
            }

            function closeTeaserModal() {
                if (!teaserModal || !teaserIframe || !teaserVideo) return;
                teaserIframe.src = '';
                teaserVideo.pause();
                teaserVideo.removeAttribute('src');
                teaserVideo.classList.add('hidden');
                ...
                teaserModal.classList.remove('flex');
            ...
            document.querySelectorAll('[data-teaser-modal]').forEach(function (trigger) {
                trigger.addEventListener('click', function (e) {
                    e.preventDefault();
                    try {
                        var payload = JSON.parse(trigger.getAttribute('data-teaser-modal') || '{}');
                        openTeaserModal(payload);
                    } catch (_) {
                        openTeaserModal({ title: '', videoUrl: '' });
                    }
                });
            });
            ...
            document.addEventListener('keydown', function (e) {
                if (e.key === 'Escape' && teaserModal && !teaserModal.classList.contains('hidden')) {
                    closeTeaserModal();
                }
